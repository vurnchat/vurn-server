...
<h1 align="center">🔐 VurnChat Server</h1>

<p align="center">
  <strong>Post-Quantum Encrypted P2P Messenger<br>Quantum-resistant. Fully distributed. Blind relay server.</strong>
</p>

<p align="center">
  <sub>Built entirely in Rust — from cryptography to networking —<br>backed by a distributed P2P network with Kademlia DHT.<br>
  → <a href="https://github.com/vurnchat/vurn-web"><code>vurn-web</code></a> — browser client (WASM, Leptos)</sub>
</p>
...
│                                        │       │                                        │
│  ┌─────────────────────────────────┐   │libp2p │  ┌─────────────────────────────────┐   │
│  │ Kademlia DHT                    │   │ Noise │  │ Kademlia DHT                    │   │
│  │ GossipSub                       │   │ TCP   │  │ GossipSub                       │   │
│  │ Identify / Ping                 │   │       │  │ Identify / Ping                 │   │
│  │ Relay / DCUtR (NAT traversal)   │   │       │  │ Relay / DCUtR (NAT traversal)   │   │
│  │                                 │   │       │  │                                 │   │
│  │ MailboxManager (Sled backup)    │   │       │  │ MailboxManager (Sled backup)    │   │
│  └─────────────────────────────────┘   │       │  └─────────────────────────────────┘   │
│                                        │       │                                        │
│  WS Gateway (:9000)                    │       │  WS Gateway (:9001)                    │
│  Health: /health                       │       │  Health: /health                       │
│  Rate limiting: 10/s                   │       │  Rate limiting: 10/s                   │
│  DHT profiles (vup_)                   │       │  DHT profiles (vup_)                   │
│  Peer store persisted                  │       │  Peer store persisted                  │
└───────────────────────────────────────┘       └───────────────────────────────────────┘
```
...
Both `FetchingIndex` and `CollectingMessages` have **10-second deadlines**:
...
Encrypted profiles, contacts, and message backups are stored in **Sled**,
encrypted with AES-256-GCM using a key derived via PBKDF2-HMAC-SHA256 (600k iterations).
...
cargo run -- --cert cert.pem --key key.pem --port 443

# Join a P2P network
cargo run -- --bootstrap /ip4/1.2.3.4/tcp/9001

# Custom P2P listen address
cargo run -- --listen-p2p /ip4/0.0.0.0/tcp/9002
```
...
```
VurnChat/
├── Cargo.toml                    # Workspace root (core + server)
...
│   └── logrotate.conf            # Log rotation for /var/log/vurn/
│
...
| **Local persistence** | Sled, AES-256-GCM encrypted |
| **Randomness** | OS entropy (`/dev/urandom`) |
...
### Phase 1 — Reliability ✅
- [x] **WS auto-reconnect** — exponential backoff, message queue, reconnection banner
...
- [ ] **Docker image** — *planned* (curl|sh install is lighter)
- [ ] **Public bootstrap nodes** — *planned* (needs infrastructure)
